...
function parseCSV(path) {
  ...
  return lines.map(line => {
    const [date, home, homeForm, away, awayForm, hs, as, ho, doo, ao] = line.split(',');
    return { date, home: home.trim(), away: away.trim(), hs: +hs, as: +as, ho: +ho, draw: +doo, ao: +ao };
  });
}
...
// All matches extracted from screenshots (date, home, away, homeScore, awayScore, homeOdds, drawOdds, awayOdds)
// CRITICAL: Only 2024 dates, deduplicated
const screenshots = [
  // === Jan 26 ~ ===
  ...
  ['2024-03-03','Sport Boys','Deportivo Binacional',2,2,2.05,3.25,3.30],

  // === Mar 8 ~ (from Mar 1 screen) ===
  ...
  ['2024-09-29','Melgar','Comerciantes Unidos',0,0,1.28,5.25,7.00],
...
for (const s of screenshots) {
  const key = `${s[0]}|${norm(s[1])}|${norm(s[2])}`;
  if (!seen.has(key)) {
    seen.add(key);
    unique.push(s);
  }
...
const csvData = parseCSV('data/footystats-results-2024.csv');

console.log(`\n=== AUDITORÍA 2024: CAPTURAS vs CSV ===`);
console.log(`Capturas de pantalla: ${unique.length} partidos únicos`);
console.log(`CSV original: ${csvData.length} partidos`);

// Build CSV lookup
const csvLookup = new Map();
for (const c of csvData) {
  const key = `${c.date}|${norm(c.home)}|${norm(c.away)}`;
  csvLookup.set(key, c);
}

let missing = 0, found = 0, scoreErrors = 0, oddsErrors = 0;
const missingList = [], scoreErrorList = [], oddsErrorList = [];

for (const [date, home, away, hs, as, ho, draw, ao] of unique) {
  const key = `${date}|${norm(home)}|${norm(away)}`;
  const match = csvLookup.get(key);

  if (!match) {
    missing++;
    missingList.push(`${date}: ${home} ${hs}-${as} ${away}`);
  } else {
    found++;
    if (match.hs !== hs || match.as !== as) {
      scoreErrors++;
      scoreErrorList.push(`${date}: ${home} ${hs}-${as} ${away} | CSV: ${match.hs}-${match.as}`);
    }
    if (Math.abs(match.ho - ho) > 0.02 || Math.abs(match.draw - draw) > 0.02 || Math.abs(match.ao - ao) > 0.02) {
      oddsErrors++;
      oddsErrorList.push(`${date}: ${home} vs ${away} | Screen: ${ho}/${draw}/${ao} | CSV: ${match.ho}/${match.draw}/${match.ao}`);
    }
  }
}

// Find extras in CSV not in screenshots
const screenKeys = new Set(unique.map(s => `${s[0]}|${norm(s[1])}|${norm(s[2])}`));
let extraInCSV = 0;
const extraList = [];
for (const c of csvData) {
  const key = `${c.date}|${norm(c.home)}|${norm(c.away)}`;
  if (!screenKeys.has(key)) {
    extraInCSV++;
    extraList.push(`${c.date}: ${c.home} ${c.hs}-${c.as} ${c.away}`);
  }
}

console.log(`\n--- RESULTADOS ---`);
console.log(`Encontrados en ambas fuentes: ${found}`);
console.log(`En capturas pero NO en CSV: ${missing}`);
console.log(`En CSV pero NO en capturas: ${extraInCSV}`);
console.log(`Errores de marcador: ${scoreErrors}`);
console.log(`Errores de cuotas: ${oddsErrors}`);

if (missingList.length > 0) {
  console.log(`\n--- PARTIDOS FALTANTES EN CSV (${missingList.length}) ---`);
  missingList.forEach(m => console.log(`  ${m}`));
}

if (scoreErrorList.length > 0) {
  console.log(`\n--- ERRORES DE MARCADOR (${scoreErrorList.length}) ---`);
  scoreErrorList.forEach(s => console.log(`  ${s}`));
}

if (extraList.length > 0) {
  console.log(`\n--- PARTIDOS EXTRA EN CSV (no en capturas) (${extraList.length}) ---`);
  extraList.forEach(e => console.log(`  ${e}`));
}

const accuracy = found > 0 ? ((found - scoreErrors) / found * 100).toFixed(1) : 0;
console.log(`\n--- RESUMEN ---`);
console.log(`Precisión de marcadores: ${accuracy}% (${found - scoreErrors}/${found} correctos)`);
console.log(`Tasa de errores total: ${((scoreErrors + missing) / unique.length * 100).toFixed(1)}%`);

// Count unique teams in CSV that are NOT in the screenshot team list
const allScreenTeams = new Set();
for (const s of unique) {
  allScreenTeams.add(norm(s[1]));
  allScreenTeams.add(norm(s[2]));
}
const csvOnlyTeams = new Set();
for (const c of csvData) {
  if (!allScreenTeams.has(norm(c.home))) csvOnlyTeams.add(c.home);
  if (!allScreenTeams.has(norm(c.away))) csvOnlyTeams.add(c.away);
}
if (csvOnlyTeams.size > 0) {
  console.log(`\n--- EQUIPOS EN CSV PERO NO EN CAPTURAS ---`);
  csvOnlyTeams.forEach(t => console.log(`  ${t}`));
}
